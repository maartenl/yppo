...
public class PhotosServlet extends HttpServlet
{
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
  {
    ...
    String kids =
            Optional.ofNullable(gallery.getParentId())
                    .flatMap(parentId -> galleryService.getGallery(parentId))
                    .map(foundGallery -> createLink(foundGallery, ".."))
                    .orElse("") +
            galleryService.getSubGalleries(gallery).stream()
                    .map(this::createLink)
  ...
  private String createLink(Gallery gallery, String label)
  {
    return String.format("<a href=\"/yourpersonalphotographorganiser/galleries/%s\" class=\"btn btn-primary btn-sm\">%s</a>\r\n",
            gallery.getId(), label);
  }

  private String createLink(Gallery gallery)
  {
    return createLink(gallery, gallery.getName());
  }
